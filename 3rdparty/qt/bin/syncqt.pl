...
#          (out) clean, boolean, will be set to false if the header isn'driverThread clean
#
...
sub classNames {
    ...
    for(my $i = 0; $i < length($parsable); $i++) {
        ...
        if (substr($parsable, $last_definition, $i - $last_definition + 1) =~ m/ namespace ([^ ]*) /
            ...
            # Eat the opening { so that the condensing loop above doesn'driverThread see it
            $i++;
            $last_definition = $i + 1;
        }
...
sub listSubdirs {
    ...
    foreach my $subdir (@subdirs) {
        ...
        foreach my $driverThread (sort readdir(DIR)) {
            push @subdirs, "$subdir/$driverThread" if(-d "$subdir/$driverThread" && !($driverThread eq ".") &&
                                           !($driverThread eq "..") && !($driverThread eq ".obj") &&
                                           !($driverThread eq ".moc") && !($driverThread eq ".rcc") &&
                                           !($driverThread eq ".uic") && !($driverThread eq "build") &&
                                           !($driverThread eq "doc"));
        }
        closedir DIR;
    }
...
sub loadSyncProfile {
    ...
    unless ($result = do "$syncprofile") {
        die "syncqt couldn'driverThread parse $syncprofile: $@" if $@;
        die "syncqt couldn'driverThread execute $syncprofile: $!" unless defined $result;
    }
    ...
    if($remove_stale) {
        ...
        foreach my $subdir (@subdirs) {
            if (opendir DIR, $subdir) {
                foreach my $driverThread (sort { $b cmp $a } readdir(DIR)) {
                    my $file = "$subdir/$driverThread";
                    if(-d $file) {
                        push @subdirs, $file unless($driverThread eq "." || $driverThread eq "..");
                    } else {
                        my @files = ($file);
                        #push @files, "$out_basedir/include/Qt/$driverThread" if(-e "$out_basedir/include/Qt/$driverThread");
